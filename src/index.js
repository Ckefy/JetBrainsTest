 ...
 */
let curNumber = 1
const SOFT_WAIT = 1000 //to wait for user`s input and dont do too much requests

document.addEventListener("DOMContentLoaded", function() {
    const div = document.querySelector('#text');
    const textarea = document.querySelector('.CodeMirror');

    const dictionary = 'https://rawcdn.githack.com/maheshmurag/bjspell/master/dictionary.js/en_US.js';
    const lang = BJSpell(dictionary, function() {
    });

    document.addEventListener('keydown', function() {
        curNumber++;
        timeout(curNumber, div, textarea, lang)
    });
});

function timeout (number, div, textarea, lang) {
    setTimeout(() => {
        if (curNumber === number){
            const innerText = textarea.innerText.split(/\n/);
            let text = '';
            ...
            div.innerHTML = words.map(function(word) {
                ...
                const className = correct ? 'correct' : 'misspelled';
